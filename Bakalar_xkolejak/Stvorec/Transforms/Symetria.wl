...
StvorecSymetriaMatrix::usage = 
    "StvorecSymetriaMatrix[a_, b_, c_] returns the transformation matrix for symmetry with respect to line ax + by + c = 0.";


StvorecSymetriaMatrix[a_, b_, c_] := 
    CalculateSymmetryMatrix[a, b, c];
    

SymetriaOs::usage = "SymetriaOs je glob\[AAcute]lna premenn\[AAcute] pre os symetrie {a, b, c}, ktor\[AAcute] sa pou\[ZHacek]\[IAcute]va v StvorecSymetria funkcii.";

SymetriaMatrix::usage = "SymetriaMatrix je glob\[AAcute]lna premenn\[AAcute] pre transforma\[CHacek]n\[UAcute] maticu symetrie.";

StvorecSymetria::usage = 
    "StvorecSymetria[vertices_] zobraz\[
] interakt\[IAcute]vny tutorial pre symetriu \[SHacek]tvorca pod\:013ea priamky a vr\[AAcute]ti nov\[EAcute] vrcholy.";

StvorecSymetriaNoDisplayWithParams::usage = 
    "StvorecSymetriaNoDisplayWithParams[vertices_] z\[IAcute]ska parametre symetrie a aplikuje ich bez zobrazovania.";

Begin["`Private`"];


SymetriaOs = {1, 1, 0};  
SymetriaMatrix = {{0, -1, 0}, {-1, 0, 0}, {0, 0, 1}}; 
...
NormalizeVertices[vertices_] := 
    Module[{normalizedVertices, processedVertices},
        ...
        If[Length[normalizedVertices] < 4,
...
StvorecSymetriaNoDisplayWithParams[vertices_] := 
    Module[{axis, result, lineParams, a, b, c},
        ...
        If[StvorecHardBalik`Transforms`Symetria`SymetriaOs =!= Null,
            ...
            _, 
                ...
                If[ListQ[axis] && Length[axis] == 3,
                    ...
                    Module[{reflectFunc},
                        ...
                        reflectFunc = If[
                            MemberQ[Names["StvorecHardBalik`Transforms`Symetria`Private`*"], 
                                  "ReflectPointOverLine"],
                            StvorecHardBalik`Transforms`Symetria`Private`ReflectPointOverLine,
                    ...
                    Module[{defaultAxis = {1, 1, 0}},  
                        ...
                        Map[StvorecHardBalik`Transforms`Symetria`Private`ReflectPointOverLine[
...
GenerateSymmetryLineFromGlobal[vertices_] := 
    Module[{a, b, c, lineType, popis, targetVertices, xIntercept, yIntercept},
        ...
        If[StvorecHardBalik`Transforms`Symetria`SymetriaOs =!= Null,
            ...
            Switch[StvorecHardBalik`Transforms`Symetria`SymetriaOs,
                "os x", 
                    a = 0; b = 1; c = 0;
                    lineType = "os x";
                ...
                _,
                    ...
                    If[ListQ[StvorecHardBalik`Transforms`Symetria`SymetriaOs] && 
                       Length[StvorecHardBalik`Transforms`Symetria`SymetriaOs] == 3,
                        {a, b, c} = StvorecHardBalik`Transforms`Symetria`SymetriaOs;
                        lineType = "v\[SHacek]eobecn\[AAcute] priamka";
...
StvorecSymetria[inputVertices_] := 
    Module[{normalizedVertices, symmetryParams, outputVertices, 
        ...
        StvorecHardBalik`Transforms`Symetria`SymetriaMatrix = transformMatrix;
        ...
        Print[Style["\[CapitalSHacek]tvorec po symetrii A'B'C'D':", Red, Bold]];
        ...
        Print[Style["\[Bullet] \[CapitalCHacek]erven\[EAcute]", Red], " body a \[CHacek]iary: \[CapitalSHacek]tvorec po symetrii"];
