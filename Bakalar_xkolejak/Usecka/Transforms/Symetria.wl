...
BeginPackage["UseckaHardBalik`Transforms`Symetria`", {"UseckaHardBalik`"}];


UseckaSymetriaMatrix::usage = 
    "UseckaSymetriaMatrix[a_, b_, c_] returns the transformation matrix for symmetry with respect to line ax + by + c = 0.";


UseckaSymetriaMatrix[a_, b_, c_] := 
    CalculateSymmetryMatrix[a, b, c];
    

SymetriaOs::usage = "SymetriaOs je glob\[AAcute]lna premenn\[AAcute] pre os symetrie {a, b, c}, ktor\[AAcute] sa pou\[ZHacek]\[IAcute]va v UseckaSymetria funkcii.";

SymetriaMatrix::usage = "SymetriaMatrix obsahuje aktu\[AAcute]lnu transforma\[CHacek]n\[UAcute] maticu pre symetriu.";

UseckaSymetria::usage = 
    "UseckaSymetria[vertices_] zobraz\[IAcute] interakt\[IAcute]vny tutorial pre symetriu \[UAcute]se\[CHacek]ky pod\:013ea priamky a vr\[AAcute]ti nov\[EAcute] vrcholy.";

UseckaSymetriaNoDisplayWithParams::usage = 
    "UseckaSymetriaNoDisplayWithParams[vertices_] z\[IAcute]ska parametre symetrie a aplikuje ich bez zobrazovania.";

Begin["`Private`"];


SymetriaOs = {1, 1, 0};  
SymetriaMatrix = {{1, 0, 0}, {0, 1, 0}, {0, 0, 1}}; 
...
NormalizeVertices[vertices_] := 
    Module[{normalizedVertices, processedVertices},
                ...
                If[Length[DeleteDuplicates[Length /@ vertices]] > 1,
                    ...
                    Block[{cleanedVertices = Select[vertices, VectorQ]},
                        If[Length[cleanedVertices] >= 2,
                            cleanedVertices[[1;;2]],
                            Message[UseckaSymetria::invalidInput]; 
                            Abort[]
                        ]
                    ],
        ...
        If[Length[normalizedVertices] < 2,
            Message[UseckaSymetria::insufficientVertices];
            Abort[]
        ];
...
UseckaSymetriaNoDisplayWithParams[vertices_] := 
    Module[{axis, result, lineParams, a, b, c},
        ...
        If[UseckaHardBalik`Transforms`Symetria`SymetriaOs =!= Null,
            ...
            _, 
                ...
                If[ListQ[axis] && Length[axis] == 3,
                    ...
                    Module[{reflectFunc},
                        ...
                        reflectFunc = If[
                            MemberQ[Names["UseckaHardBalik`Transforms`Symetria`Private`*"], 
                                  "ReflectPointOverLine"],
                            UseckaHardBalik`Transforms`Symetria`Private`ReflectPointOverLine,
                    ...
                    Module[{defaultAxis = {1, 1, 0}},  
                        ...
                        Map[UseckaHardBalik`Transforms`Symetria`Private`ReflectPointOverLine[
        ...
        If[ListQ[axis] && Length[axis] == 3,
            UseckaHardBalik`Transforms`Symetria`SymetriaMatrix = 
                CalculateSymmetryMatrix[axis[[1]], axis[[2]], axis[[3]]];
        ];
...
GenerateSymmetryLineFromGlobal[vertices_] := 
    Module[{a, b, c, lineType, popis, targetVertices, xIntercept, yIntercept},
        ...
        If[UseckaHardBalik`Transforms`Symetria`SymetriaOs =!= Null,
            ...
            Switch[UseckaHardBalik`Transforms`Symetria`SymetriaOs,
                "os x", 
                    a = 0; b = 1; c = 0;
                    lineType = "os x";
                ...
                _,
                    ...
                    If[ListQ[UseckaHardBalik`Transforms`Symetria`SymetriaOs] && 
                       Length[UseckaHardBalik`Transforms`Symetria`SymetriaOs] == 3,
                        {a, b, c} = UseckaHardBalik`Transforms`Symetria`SymetriaOs;
                        lineType = "v\[SHacek]eobecn\[AAcute] priamka";
...
UseckaSymetria[inputVertices_] := 
    Module[{normalizedVertices, symmetryParams, outputVertices, 
        ...
        UseckaHardBalik`Transforms`Symetria`SymetriaMatrix = transformMatrix;
        ...
        Print[Style[
            "GEOMETRICK\[CapitalEAcute] TRANSFORM\[CapitalAAcute]CIE \[CapitalUAcute]SE\[CapitalCHacek]KY - SYMETRIA", 
            Bold, 16]];
        Print[Style["========================================", Bold]];
        
        
        Print[Style["\nZADANIE:", Bold, 14]];
        Print["Majme \[UAcute]se\[CHacek]ku AB s vrcholmi:"];
        ...
        Print[Style["\[CapitalUAcute]se\[CHacek]ka po symetrii A'B':", Red, Bold]];
        ...
        Print[Style["\[Bullet] \[CapitalCHacek]erven\[EAcute]", Red], " body a \[CHacek]iary: \[CapitalUAcute]se\[CHacek]ka po symetrii"];
...
UseckaSymetria::invalidInput = "Neplatn\[YAcute] vstup. O\[CHacek]ak\[AAcute]va sa matica alebo zoznam s\[UAcute]radn\[IAcute]c \[UAcute]se\[CHacek]ky.";
UseckaSymetria::insufficientVertices = "Nedostato\[CHacek]n\[YAcute] po\[CHacek]et vrcholov. S\[UAcute] potrebn\[EAcute] aspo\[NHacek] 2 vrcholy.";
