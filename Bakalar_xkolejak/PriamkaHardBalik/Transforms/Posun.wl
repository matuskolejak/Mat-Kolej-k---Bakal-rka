...
BeginPackage["PriamkaHardBalik`Transforms`Posun`", {"PriamkaHardBalik`"}];

PriamkaPosun::usage = 
    "PriamkaPosun[vertices_] zobraz\[IAcute] interakt\[IAcute]vny tutori\[AAcute]l pre posun priamky a vr\[AAcute]ti nov\[EAcute] body.";
...
NormalizeVertices[vertices_] := 
    Module[{processedVertices},
                ...
                If[Length[DeleteDuplicates[Length /@ vertices]] > 1,
                    ...
                    Block[{cleanedVertices = Select[vertices, VectorQ]},
                        If[Length[cleanedVertices] >= 2,
                            cleanedVertices[[1;;2]],
                            Message[PriamkaPosun::invalidInput]; 
                            Abort[]
                        ]
                    ],
        ...
        If[Length[processedVertices] < 2,
            Message[PriamkaPosun::insufficientVertices];
            Abort[]
        ];
        
        (* D\[CapitalOHat]LE\[CapitalZHacek]IT\[CapitalAAcute] ZMENA: Ne\[SHacek]k\[AAcute]lujeme vstupn\[EAcute] body, aby sme zachovali presn\[EAcute] hodnoty 
           z predch\[AAcute]dzaj\[UAcute]cej transform\[AAcute]cie *)
        
        (* Vraciame presn\[EAcute] hodnoty bez upravovania *)
...
(* Funkcia na generovanie parametrov posunu *)
    ...
    Module[{dx, dy, valid = False, newVertices, 
        ...
        While[!valid,
            ...
            If[narocnost < 0.7, 
                ...
                (* Klasick\[EAcute] posuny *)
                dx = RandomChoice[{-5, -4, -3, -2, 2, 3, 4, 5}];
                dy = RandomChoice[{-5, -4, -3, -2, 2, 3, 4, 5}]
            ];
            
            (* V\[YAcute]po\[CHacek]et nov\[YAcute]ch bodov po posune - PRESN\[CapitalEAcute] HODNOTY BEZ ZAOKR\[CapitalUAcute]H\:013dOVANIA *)
            ...
            valid = 
                (* Kontrola, \[CHacek]i v\[YAcute]sledn\[EAcute] body s\[UAcute] v bezpe\[CHacek]nej oblasti *)
                AllTrue[newVertices, (Abs[#[[1]]] <= 14 && Abs[#[[2]]] <= 14) &] &&
                (* Kontrola minim\[AAcute]lnej vzdialenosti posunu - aspo\[NHacek] o 3 jednotky *)
                Sqrt[dx^2 + dy^2] >= 3 &&
                (* Kontrola, \[CHacek]i inverzn\[EAcute] body sa rovnaj\[UAcute] p\[OHat]vodn\[YAcute]m - PRESN\[CapitalEAcute] POROVNANIE *)
                Total[Flatten[Abs[allVertices - invNewVertices]]] < 0.00001 &&
                (* Zabr\[AAcute]nenie prieniku priamok *)
                Min[
                    Table[
    ...
    Module[{allVertices, xMin, xMax, yMin, yMax, rangeBuffer, xRange, yRange, 
        ...
        labelOffsets = Table[
            Module[{originalVertex = originalVertices[[i]], 
                ...
                (* Upravi\[THacek] offset pod\:013eea polohy bodov *)
        ...
        Graphics[{
            ...
            (* Posunut\[AAcute] priamka - \[CHacek]erven\[AAcute] *)
...
PriamkaPosun[inputVertices_] := 
    Module[{normalizedVertices, translationParams, outputVertices, dx, dy, invDx, invDy},
        ...
        (* Tu skontrolujeme, \[CHacek]i pou\[ZHacek]\[IAcute]vame priamo zadan\[EAcute] body, ak \[AAcute]no, vypo\[CHacek]\[IAcute]tame posun pomocou nich *)
        If[Length[inputVertices] === 2 && validLineQ @@ inputVertices,
            (* Kontrola, \[CHacek]i m\[OHat]\[ZHacek]eme vypo\[CHacek]\[IAcute]ta\[THacek] posun z existuj\[UAcute]cich bodov *)
            (* Vygenerujeme n\[AAcute]hodn\[EAcute] cel\[EAcute] \[CHacek]\[IAcute]sla pre dx a dy *)
            dx = RandomChoice[{-7, -6, -5, -4, -3, 3, 4, 5, 6, 7}];
            dy = RandomChoice[{-7, -6, -5, -4, -3, 3, 4, 5, 6, 7}];
            
            (* V\[YAcute]po\[CHacek]et nov\[YAcute]ch bodov s presn\[YAcute]mi hodnotami *)
            outputVertices = Map[{#[[1]] + dx, #[[2]] + dy} &, normalizedVertices];
            
            (* Kontrola, \[CHacek]i s\[UAcute] v\[SHacek]etky body v bezpe\[CHacek]nej oblasti *)
            ...
            (* Ak nie s\[UAcute] validn\[EAcute] body, generujeme nov\[EAcute] parametre posunu *)
            translationParams = GetTranslationParameters[normalizedVertices];
            dx = translationParams[[1]];
            dy = translationParams[[2]];
            outputVertices = translationParams[[4]];
        ...
        (* Nadpis *)
        Print[Style[
            "GEOMETRICK\[CapitalEAcute] TRANSFORM\[CapitalAAcute]CIE PRIAMKY - POSUN", 
            Bold, 16]];
        Print[Style["========================================", Bold]];
        ...
        Print["\nVykonajte posun priamky v 2D priestore pomocou vektora posunu: \[CapitalDelta] = [", 
        ...
        Print["\[Bullet] \[CapitalCHacek]erven\[EAcute] body a \[CHacek]iary: Posunut\[AAcute] priamka"];
        Print["\[Bullet] Zelen\[AAcute] \[SHacek]\[IAcute]pka: Vektor posunu"];
        Print["\[Bullet] Zelen\[EAcute] preru\[SHacek]ovan\[EAcute] \[CHacek]iary: Posun jednotliv\[YAcute]ch bodov"];
        ...
        (* Vr\[AAcute]tenie nov\[YAcute]ch bodov pre pr\[IAcute]padn\[EAcute] \[DHacek]al\[SHacek]ie transform\[AAcute]cie - PRESN\[CapitalEAcute] HODNOTY *)
        outputVertices
    ];

(* Defin\[IAcute]cia chybov\[YAcute]ch spr\[AAcute]v *)
PriamkaPosun::invalidInput = "Neplatn\[YAcute] vstup. O\[CHacek]ak\[AAcute]va sa matica alebo zoznam s\[UAcute]radn\[IAcute]c priamky.";
PriamkaPosun::insufficientVertices = "Nedostato\[CHacek]n\[YAcute] po\[CHacek]et bodov. S\[UAcute] potrebn\[EAcute] aspo\[NHacek] 2 body.";
