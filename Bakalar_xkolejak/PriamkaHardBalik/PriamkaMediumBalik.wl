...
BeginPackage["PriamkaMediumBalik`"];

(* Export verejn\[YAcute]ch symbolov *)
PriamkaDvojitaTransformacia::usage = 
    "PriamkaDvojitaTransformacia[] umo\[ZHacek]n\[IAcute] postupn\[YAcute] v\[YAcute]ber a aplik\[AAcute]ciu dvoch transform\[AAcute]ci\[IAcute].";

PriamkaDvojitaTransformaciaSVysledkom::usage = 
    "PriamkaDvojitaTransformaciaSVysledkom[] umo\[ZHacek]n\[IAcute] postupn\[YAcute] v\[YAcute]ber a aplik\[AAcute]ciu dvoch transform\[AAcute]ci\[IAcute] a zobraz\[IAcute] s\[UAcute]hrnn\[YAcute] v\[YAcute]sledok.";
...
PriamkaDvojitaTransformacia::infinityerr = 
...
GenerateInitialLine[] := Module[{
  ...
  result = Check[
    Block[{$MessagePreprint = (Message[PriamkaDvojitaTransformacia::infinityerr]; #)&},
      While[True,
...
DisplayTransformationSequence[pociatocna_, druha_, finalna_, prva_, druha_] := Module[{},
    ...
    Print["Body priamky A''B'':"];
    Print[Style[MatrixForm[finalna], Orange]];
    DisplayPriamkaProperties[finalna, Orange];
];

(* Vytvorenie s\[UAcute]hrnnej vizualiz\[AAcute]cie dvoch transform\[AAcute]ci\[IAcute] *)
CreateSummaryVisualization[pociatocna_, druha_, finalna_] := 
    Module[{
        ...
        allVertices = Join[N[pociatocna], N[druha], N[finalna]];
        ...
        Graphics[{
            ...
            {orange, PointSize[0.02], Point[N[finalna]]},
            ...
            {orange, Text[Style["A''", Bold, 12], 
                getLabelPosition[N[finalna[[1]]], N[finalna[[2]]], orange], {0, 0}]},
            {orange, Text[Style["B''", Bold, 12], 
                getLabelPosition[N[finalna[[2]]], N[finalna[[1]]], orange], {0, 0}]},
        ...
        ImageSize -> 600,
        ImagePadding -> 30,
        PlotLabel -> Style["Postupn\[AAcute] aplik\[AAcute]cia dvoch transform\[AAcute]ci\[IAcute] priamky", Bold, 14]
        ]
    ];
    
(* Vytvorenie textovej legendy pre s\[UAcute]hrnn\[UAcute] vizualiz\[AAcute]ciu *)
CreateSummaryLegend[prva_, druha_] := 
    Module[{},
...
DisplayTransformationSummary[pociatocnaPriamka_, druhaPriamka_, finalnaPriamka_, prva_, druha_] := 
  Module[{
      blue = RGBColor[0, 0.4, 0.8],
      green = RGBColor[0.2, 0.8, 0.2],
      orange = RGBColor[1, 0.6, 0],
    ...
    Print[Row[{Style["Po druhej transform\[AAcute]cii (", Bold], Style[druha, Bold, orange], Style["): ", Bold], Style[MatrixForm[simplifiedFinalPriamka], orange, Bold]}]];
  ];
  
(* Upraven\[AAcute] hlavn\[AAcute] funkcia - za\[CHacek]iatok s \[CHacek]iernym nadpisom *)
PriamkaDvojitaTransformacia[] := Module[{
    pociatocnaPriamka,
    prvaTransformacia,
    druhaPriamka,
    druhaTransformacia,
    ...
    Print[Style["DVOJIT\[CapitalAAcute] GEOMETRICK\[CapitalAAcute] TRANSFORM\[CapitalAAcute]CIA PRIAMKY", Bold, 18]];
    Print[Style["==========================================", Bold]];
    ...
    finalnaPriamka = Switch[druhaTransformacia,
    ...
    Print[CreateSummaryVisualization[pociatocnaPriamka, druhaPriamka, finalnaPriamka]];
    
    (* Zobrazenie legendy *)
    CreateSummaryLegend[prvaTransformacia, druhaTransformacia];
    ...
    {pociatocnaPriamka, druhaPriamka, finalnaPriamka, 
...
PriamkaDvojitaTransformaciaSVysledkom[] := Module[{result},
   (* Pou\[ZHacek]itie Quiet na potla\[CHacek]enie chybov\[YAcute]ch spr\[AAcute]v *)
   result = Quiet[PriamkaDvojitaTransformacia[], {Power::infy, Infinity::indet}];
...
EndPackage[]; (* Uzavretie PriamkaMediumBalik package *)
