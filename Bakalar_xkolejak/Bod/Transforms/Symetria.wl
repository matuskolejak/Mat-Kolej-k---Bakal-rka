...
BodSymetriaMatrix::usage = 
    "BodSymetriaMatrix[a_, b_, c_] returns the transformation matrix for symmetry with respect to line ax + by + c = 0.";


BodSymetriaMatrix[a_, b_, c_] := 
    CalculateSymmetryMatrix[a, b, c];
    

SymetriaOs::usage = "SymetriaOs je glob\[AAcute]lna premenn\[AAcute] pre os symetrie {a, b, c}, ktor\[AAcute] sa pou\[ZHacek]\[IAcute]va v BodSymetria funkcii.";


SymetriaMatrix::usage = "SymetriaMatrix obsahuje posledn\[UAcute] vypo\[CHacek]\[IAcute]tan\[UAcute] transforma\[CHacek]n\[UAcute] maticu pre symetriu.";

BodSymetria::usage = 
    "BodSymetria[point_] zobraz\[IAcute] interakt\[IAcute]vny tutorial pre symetriu bodu pod\:013ea priamky a vr\[AAcute]ti nov\[YAcute] bod.";

BodSymetriaNoDisplayWithParams::usage = 
    "BodSymetriaNoDisplayWithParams[point_] z\[IAcute]ska parametre symetrie a aplikuje ich bez zobrazovania.";
...
GenerateSymmetryLine[point_] := 
    Module[{a, b, c, validLine = False, 
            ...
            initialPoint, targetPoint, specialLines, complexLines = {}},
        
        
        SeedRandom[Hash[{ToString[Date[]], ToString[point], RandomReal[]}]];
        ...
        While[!validLine && attempts < maxAttempts,
            ...
            targetPoint = ReflectPointOverLine[initialPoint, a, b, c];
            
            
            validLine = True;
            
           
            validLine = validLine && (Abs[N[targetPoint[[1]]]] <= 14 && Abs[N[targetPoint[[2]]]] <= 14);
            
            
            If[validLine, 
                validLine = N[EuclideanDistance[initialPoint, targetPoint]] >= 1;
            ];
        ];
        ...
        {a, b, c, lineType, popis, targetPoint, xIntercept, yIntercept}
    ];
...
CreateVisualization[originalPoint_, finalPoint_, a_, b_, c_, lineType_, xIntercept_, yIntercept_] := 
    Module[{allPoints, xMin, xMax, yMin, yMax, rangeBuffer, xRange, yRange, 
        ...
        labelOffsets = {
            Module[{offset = {0.7, 0.7}},
                
                If[N[originalPoint[[1]]] > 8, offset[[1]] = -1.0];
                If[N[originalPoint[[2]]] > 8, offset[[2]] = -1.0];
                offset
            ],
            Module[{offset = {0.7, 0.7}},
                
                If[N[finalPoint[[1]]] > 8, offset[[1]] = -1.0];
                If[N[finalPoint[[2]]] > 8, offset[[2]] = -1.0];
                offset
            ]
        };
        ...
        Graphics[{
            ...
            Blue, PointSize[0.025], Point[originalPoint],
            
            
            Red, PointSize[0.025], Point[finalPoint],
            ...
            brightGreen, Dashed,
            Line[{originalPoint, finalPoint}],
            
            
            White, 
            Disk[originalPoint, 0.45],
            Disk[finalPoint, 0.45],
            ...
            Text[Style["P", Blue, Bold, 16], originalPoint + labelOffsets[[1]]],
            Text[Style["P'", Red, Bold, 16], finalPoint + labelOffsets[[2]]],
...
BodSymetriaNoDisplayWithParams[point_] := 
    Module[{axis, result, lineParams, a, b, c},
        
        If[BodHardBalik`Transforms`Symetria`SymetriaOs =!= Null,
            axis = BodHardBalik`Transforms`Symetria`SymetriaOs;
        ,
            ...
            _, 
                ...
                If[ListQ[axis] && Length[axis] == 3,
                    ...
                    Module[{reflectFunc},
                        ...
                        reflectFunc = If[
                            MemberQ[Names["BodHardBalik`Transforms`Symetria`Private`*"], 
                                  "ReflectPointOverLine"],
                            BodHardBalik`Transforms`Symetria`Private`ReflectPointOverLine,
                        ...
                        reflectFunc[point, lineParams[[1]], lineParams[[2]], lineParams[[3]]]
                    ],
                    ...
                    Module[{defaultAxis = {1, 1, 0}},  
                        ...
                        BodHardBalik`Transforms`Symetria`Private`ReflectPointOverLine[
                            point, defaultAxis[[1]], defaultAxis[[2]], defaultAxis[[3]]]
                    ]
                ]
        ];
        ...
        BodHardBalik`Transforms`Symetria`SymetriaOs = axis;
...
GenerateSymmetryLineFromGlobal[point_] := 
    Module[{a, b, c, lineType, popis, targetPoint, xIntercept, yIntercept},
        
        If[BodHardBalik`Transforms`Symetria`SymetriaOs =!= Null,
            
            Switch[BodHardBalik`Transforms`Symetria`SymetriaOs,
                "os x", 
                    a = 0; b = 1; c = 0;
                    lineType = "os x";
                ...
                _,
                    ...
                    If[ListQ[BodHardBalik`Transforms`Symetria`SymetriaOs] && 
                       Length[BodHardBalik`Transforms`Symetria`SymetriaOs] == 3,
                        {a, b, c} = BodHardBalik`Transforms`Symetria`SymetriaOs;
                        lineType = "v\[SHacek]eobecn\[AAcute] priamka";
            ...
            {a, b, c, lineType, popis, targetPoint, xIntercept, yIntercept} = GenerateSymmetryLine[point];
        ];
        
        
        targetPoint = ReflectPointOverLine[point, a, b, c];
...
BodSymetria[inputPoint_] := 
    Module[{symmetryParams, outputPoint, 
        ...
        BodHardBalik`Transforms`Symetria`SymetriaMatrix = transformMatrix;
        ...
        Print["\nMenovate\:013e: a^2 + b^2 = ", a, "^2 + ", b, "^2 = ", a^2 + b^2];
        
        
        Print[Style["\nPrv\[YAcute] riadok matice:", Bold]];
        Print["M[1,1] = (b^2 - a^2)/(a^2 + b^2) = (", b^2, " - ", a^2, ")/(", a^2 + b^2, ") = ", (b^2 - a^2)/(a^2 + b^2)];
        Print["M[1,2] = -2ab/(a^2 + b^2) = -2\[CenterDot]", a, "\[CenterDot]", b, "/(", a^2 + b^2, ") = ", -2*a*b/(a^2 + b^2)];
        Print["M[1,3] = -2ac/(a^2 + b^2) = -2\[CenterDot]", a, "\[CenterDot]", c, "/(", a^2 + b^2, ") = ", -2*a*c/(a^2 + b^2)];
        
        
        Print[Style["\nDruh\[YAcute] riadok matice:", Bold]];
        Print["M[2,1] = -2ab/(a^2 + b^2) = -2\[CenterDot]", a, "\[CenterDot]", b, "/(", a^2 + b^2, ") = ", -2*a*b/(a^2 + b^2)];
        Print["M[2,2] = (a^2 - b^2)/(a^2 + b^2) = (", a^2, " - ", b^2, ")/(", a^2 + b^2, ") = ", (a^2 - b^2)/(a^2 + b^2)];
        Print["M[2,3] = -2bc/(a^2 + b^2) = -2\[CenterDot]", b, "\[CenterDot]", c, "/(", a^2 + b^2, ") = ", -2*b*c/(a^2 + b^2)];
        ...
        intersection = CalculateIntersection[inputPoint, a, b, c];
        
        
        distance = EuclideanDistance[inputPoint, intersection];
        
        
        lineEqn = FormatLineEquation[a, b, c];
        
        
        Print[Style["\nV\[CapitalYAcute]PO\[CapitalCHacek]ET SYMETRIE BODU:", Bold, 14]];
        
        
        Module[{x = inputPoint[[1]], 
               y = inputPoint[[2]],
            ...
            Print[Style["\nV\[YAcute]sledn\[AAcute] reprezent\[AAcute]cia bodu P':", Bold]];
        ...
        Print[Style["Bod po symetrii P':", Red, Bold]];
        Print[outputPoint];
        
        
        Print[Style["\nVIZUALIZ\[CapitalAAcute]CIA TRANSFORM\[CapitalAAcute]CIE:", Bold]];
        Print[CreateVisualization[inputPoint, outputPoint, a, b, c, lineType, xIntercept, yIntercept]];
        
        
        Print[Style["\nLEGENDA:", Bold, 14]];
        Print[Style["\[Bullet] Modr\[EAcute]", Blue], " body a \[CHacek]iary: P\[OHat]vodn\[YAcute] bod"];
        Print[Style["\[Bullet] \[CapitalCHacek]erven\[EAcute]", Red], " body a \[CHacek]iary: Bod po symetrii"];
        Print[Style["\[Bullet] Zelen\[EAcute]", darkGreen], " \[CHacek]iara: Os symetrie"];
        Print[Style["\[Bullet] Zelen\[EAcute]", darkGreen], " preru\[SHacek]ovan\[EAcute] \[CHacek]iary: Spojenia zodpovedaj\[UAcute]cich si bodov"];
        ...
        Print[Row[{Style["Bod P: ", RGBColor[0.1, 0.1, 1]], inputPoint}]];
...
BodSymetria::invalidInput = "Neplatn\[YAcute] vstup. O\[CHacek]ak\[AAcute]va sa bod so s\[UAcute]radnicami {x, y}.";
