...
BeginPackage["StvorecHardBalik`Transforms`Symetria`", {"StvorecHardBalik`"}];

(* Export public symbols *)
StvorecSymetria::usage = 
    "StvorecSymetria[vertices_] zobraz\[IAcute] interakt\[IAcute]vny tutori\[AAcute]l pre symetriu \[SHacek]tvorca pod\:013ea priamky a vr\[AAcute]ti nov\[EAcute] vrcholy.";

Begin["`Private`"];

(* Pomocn\[AAcute] funkcia na normaliz\[AAcute]ciu vstupn\[YAcute]ch s\[UAcute]radn\[IAcute]c - akceptuje presne tak\[EAcute], ak\[EAcute] s\[UAcute] bez \[UAcute]prav *)
NormalizeVertices[vertices_] := 
    Module[{processedVertices},
        (* Zachovaj form\[AAcute]t vstupu presne tak, ako je bez ak\[YAcute]chko\:013evek \[UAcute]prav *)
                ...
                If[Length[DeleteDuplicates[Length /@ vertices]] > 1,
                    ...
                    Block[{cleanedVertices = Select[vertices, VectorQ]},
                        If[Length[cleanedVertices] >= 4,
                            cleanedVertices[[1;;4]],
                            Message[StvorecSymetria::invalidInput]; 
                            Abort[]
                        ]
                    ],
        ...
        If[Length[processedVertices] < 4,
            Message[StvorecSymetria::insufficientVertices];
            Abort[]
        ];
        
        (* KRITICK\[CapitalEAcute]: Zachov\[AAcute]me PRESNE p\[OHat]vodn\[YAcute] form\[AAcute]t v\[YAcute]razov bez ak\[EAcute]hoko\:013evek zjednodu\[SHacek]ovania *)
        processedVertices[[1;;4]]
    ];

(* OPRAVEN\[CapitalAAcute] FUNKCIA: Spr\[AAcute]vny v\[YAcute]po\[CHacek]et osovej s\[UAcute]mernosti bodu pod\:013ea priamky *)
ReflectPointOverLine[point_, a_, b_, c_] := 
    Module[{x, y, numerator, denominator, xNew, yNew},
        (* Pou\[ZHacek]ijeme priamo p\[OHat]vodn\[EAcute] s\[UAcute]radnice bez akejko\:013evek \[UAcute]pravy *)
        x = point[[1]];
        y = point[[2]];
        
        (* V\[YAcute]po\[CHacek]et vzdialenosti bodu od priamky *)
        numerator = a*x + b*y + c;
        denominator = a^2 + b^2;
        
        (* Presn\[YAcute] v\[YAcute]po\[CHacek]et s\[UAcute]radn\[IAcute]c obrazu - k\:013e\[UAcute]\[CHacek]ov\[YAcute] bod opravy *)
        xNew = x - 2*a*numerator/denominator;
        yNew = y - 2*b*numerator/denominator;
        
        (* D\[OHat]le\[ZHacek]it\[EAcute]: U\[ZHacek] nepou\[ZHacek]\[IAcute]vame Simplify, aby sa zachoval presn\[YAcute] form\[AAcute]t v\[YAcute]razov *)
        {xNew, yNew}
    ];
...
GenerateSymmetryLine[vertices_] := 
    Module[{a, b, c, validLine = False, allVertices, 
        ...
        (* Jednoduch\[EAcute] hodnoty koeficientov - roz\[SHacek]\[IAcute]ren\[EAcute] rozp\[ADoubleDot]tie *)
        simpleCoefficients = Join[{-5, -4, -3, -2, -1, 1, 2, 3, 4, 5}, {-1/2, -1/3, -2/3, 1/2, 1/3, 2/3}];
        
        (* Jednoduch\[EAcute] hodnoty pre c - roz\[SHacek]\[IAcute]ren\[EAcute] rozp\[ADoubleDot]tie *)
        simpleIntercepts = Join[Range[-6, 6], {-1/2, -1/3, -2/3, 1/2, 1/3, 2/3}];
        
        (* \[CapitalSHacek]peci\[AAcute]lne, zauj\[IAcute]mav\[EAcute] typy priamok pre \[SHacek]tvorec - ale budeme ich pou\[ZHacek]\[IAcute]va\[THacek] menej *)
        specialLines = {
            (* Os X: y = 0 *)
            ...
            (* Priamka y = x - diagon\[AAcute]la \[SHacek]tvrorca *)
            {1, -1, 0, "priamka y = x", "symetria pod\:013ea priamky y = x"},
            
            (* Priamka y = -x - druh\[AAcute] diagon\[AAcute]la \[SHacek]tvrorca *)
            {1, 1, 0, "priamka y = -x", "symetria pod\:013ea priamky y = -x"},
            
            (* Vodorovn\[AAcute] os symetrie \[SHacek]trvorca - bez pou\[ZHacek]itia Simplify na zachovanie form\[AAcute]tu *)
            {0, 1, -((initialVertices[[1, 2]] + initialVertices[[3, 2]])/2), 
             "vodorovn\[AAcute] os symetrie \[SHacek]tvorca", 
             "symetria pod\:013ea vodorovnej osi symetrie \[SHacek]tvorca"},
            
            (* Zvisl\[AAcute] os symetrie \[SHacek]trvorca - bez pou\[ZHacek]itia Simplify na zachovanie form\[AAcute]tu *)
            {1, 0, -((initialVertices[[1, 1]] + initialVertices[[3, 1]])/2), 
             "zvisl\[AAcute] os symetrie \[SHacek]tvorca", 
             "symetria pod\:013ea zvislej osi symetrie \[SHacek]tvorca"}
        };
        
        (* HLAVN\[CapitalYAcute] INVERZN\[CapitalYAcute] ALGORITMUS *)
        (* 1. Najprv vytvor\[IAcute]m n\[AAcute]hodn\[YAcute] cie\:013eov\[YAcute] \[SHacek]tvorec *)
        (* 2. Potom n\[AAcute]jdem symetriu, ktor\[AAcute] prevedie p\[OHat]vodn\[YAcute] na cie\:013eov\[YAcute] *)
        (* 3. Nakoniec over\[IAcute]m, \[ZHacek]e inverzn\[AAcute] transform\[AAcute]cia vr\[AAcute]ti spr\[AAcute]vne p\[OHat]vodn\[YAcute] \[SHacek]tvorec *)
        
        While[!validLine && attempts < maxAttempts,
            attempts++;
            
            (* Sk\[UAcute]sime zvoli\[THacek] parametre priamky n\[AAcute]hodne - preferujeme v\[SHacek]eobecn\[EAcute] priamky *)
            If[RandomReal[] < 0.3, (* Zn\[IAcute]\[ZHacek]en\[AAcute] \[SHacek]anca na \[SHacek]peci\[AAcute]lne priamky *)
                (* \[CapitalSHacek]peci\[AAcute]lne priamky *)
                {a, b, c, lineType, popis} = RandomChoice[specialLines];
            ,
                (* V\[SHacek]eobecn\[EAcute] priamky s n\[AAcute]hodn\[YAcute]mi koeficientami *)
            ...
            (* \[CapitalDHacek]al\[SHacek]ie kreat\[IAcute]vne priamky - prech\[AAcute]dza cez niektor\[YAcute] vrchol *)
            If[RandomReal[] < 0.2 && a == 0 && b == 0,
                (* Vyber n\[AAcute]hodn\[YAcute] vrchol *)
                Module[{randVertex, randPoint},
                    randVertex = RandomChoice[initialVertices];
                    
                    (* Vytvor priamku, ktor\[AAcute] prech\[AAcute]dza t\[YAcute]mto vrcholom *)
                    (* N\[AAcute]hodn\[YAcute] smer - ale nie pr\[IAcute]li\[SHacek] extr\[EAcute]mny *)
                    a = RandomChoice[{-3, -2, -1, 1, 2, 3}];
                    b = RandomChoice[{-3, -2, -1, 1, 2, 3}];
                    
                    (* Vypo\[CHacek]\[IAcute]taj c tak, aby priamka prech\[AAcute]dzala vrcholom *)
                    c = -(a*randVertex[[1]] + b*randVertex[[2]]);
                    ...
                    (* Aktualizuj popis *)
                    lineType = "priamka prech\[AAcute]dzaj\[UAcute]ca vrcholom " <> 
                              FromCharacterCode[64 + Position[initialVertices, randVertex][[1, 1]]];
            ...
            *)
            ...
            (* Kontrola minim\[AAcute]lnej zmeny - \[SHacek]tvrce s\[UAcute] dostato\[CHacek]ne odli\[SHacek]n\[EAcute] *)
            If[validLine, 
                validLine = Min[Flatten[Map[Function[{orig, target}, 
                    N[EuclideanDistance[orig, target]]], 
                    Transpose[{initialVertices, targetVertices}]]]] >= 0.5;
            ];
            ...
            If[validLine,
                ...
                (* 4. Overenie, \[ZHacek]e inverzn\[AAcute] transform\[AAcute]cia vr\[AAcute]ti p\[OHat]vodn\[YAcute] \[SHacek]tvorec *)
        ...
        If[!validLine,
            (* Vytvor\[IAcute]me n\[AAcute]hodn\[UAcute] priamku *)
            a = RandomChoice[{-2, -1, 1, 2}];
            b = RandomChoice[{-2, -1, 1, 2}];
            c = RandomChoice[{-3, -2, -1, 0, 1, 2, 3}];
...
CreateVisualization[originalVertices_, finalVertices_, a_, b_, c_, lineType_, xIntercept_, yIntercept_] := 
    Module[{allVertices, xMin, xMax, yMin, yMax, rangeBuffer, xRange, yRange, 
        ...
        (* Vypo\[CHacek]\[IAcute]ta\[THacek] priese\[CHacek]n\[IAcute]ky spojn\[IAcute]c p\[OHat]vodn\[YAcute]ch a nov\[YAcute]ch bodov s osou symetrie *)
        intersections = Table[
            Module[{pt1 = originalVertices[[i]], pt2 = finalVertices[[i]]},
                (* V\[YAcute]po\[CHacek]et priese\[CHacek]n\[IAcute]ka spojnice s osou symetrie *)
                Module[{t},
                    (* Stredov\[YAcute] bod spojnice - le\[ZHacek]\[IAcute] na osi symetrie *)
                    (pt1 + pt2)/2
                ]
            ],
        ...
        Graphics[{
            ...
            (* P\[OHat]vodn\[YAcute] \[SHacek]tvorec - modr\[YAcute] *)
            Blue, Thick, Opacity[0.7],
            Line[Append[originalVertices, First[originalVertices]]],
            
            (* Symetrick\[YAcute] \[SHacek]tvorec - \[CHacek]erven\[YAcute] *)
            ...
            (* Priese\[CHacek]n\[IAcute]ky spojn\[IAcute]c s osou symetrie - mal\[EAcute] zelen\[EAcute] body *)
            brightGreen, PointSize[0.01],
            Point[intersections],
...
(* OPRAVEN\[CapitalYAcute]: FormatLineEquation, aby spr\[AAcute]vne form\[AAcute]toval rovnicu priamky *)
FormatLineEquation[a_, b_, c_] := 
    Module[{eqn = ""},
        (* Vytvorit rovnicu priamky vo forme ax + by + c = 0 *)
        If[a == 0 && b == 0,
            (* Neplatn\[AAcute] priamka *)
            Return["Neplatn\[AAcute] rovnica priamky"];
        ];
        
        (* \[CapitalCHacek]as\[THacek] s x *)
        If[a != 0,
            eqn = If[a == 1, "x", If[a == -1, "-x", ToString[a] <> "x"]];
        ];
        
        (* \[CapitalCHacek]as\[THacek] s y *)
        If[b != 0,
            If[a != 0, 
                eqn = eqn <> If[b > 0, " + ", " - "];
...
(* OPRAVEN\[CapitalYAcute]: V\[YAcute]po\[CHacek]et prieseku kolmice s priamkou *)
CalculateIntersection[point_, a_, b_, c_] := 
    Module[{x0, y0, x1, y1, t},
        x0 = point[[1]];
        y0 = point[[2]];
        
        (* Parameter t pre priesek - presnej\[SHacek]\[IAcute] v\[YAcute]po\[CHacek]et *)
...
StvorecSymetria[inputVertices_] := 
    Module[{normalizedVertices, symmetryParams, outputVertices, 
        ...
        (* D\[CapitalOHat]LE\[CapitalZHacek]IT\[CapitalAAcute] ZMENA: Zachovanie presn\[YAcute]ch vstupn\[YAcute]ch s\[UAcute]radn\[IAcute]c bez normaliz\[AAcute]cie *)
        ...
        (* V\[YAcute]po\[CHacek]et vzdialianost\[IAcute] medzi p\[OHat]vodn\[YAcute]mi bodmi a priesekmi *)
        distances = MapThread[
        ...
        (* Nadpis *)
        Print[Style[
            "GEOMETRICK\[CapitalEAcute] TRANSFORM\[CapitalAAcute]CIE \[CapitalSHacek]TVORCA - SYMETRIA", 
            Bold, 16]];
        Print[Style["========================================", Bold]];
        ...
        Print["x' = x - 2a(ax + by + c)/(a^2 + b^2)"];
        Print["y' = y - 2b(ax + by + c)/(a^2 + b^2)"];
        ...
        Print["1. V\[YAcute]po\[CHacek]et v\[YAcute]razu (ax + by + c)/(a^2 + b^2) pre ka\[ZHacek]d\[YAcute] bod"];
        ...
        Table[
            Module[{x = normalizedVertices[[i, 1]], 
                ...
                Print["x' = ", x, " - 2\[CenterDot]", a, "\[CenterDot](", a, "\[CenterDot]", x, " + ", b, "\[CenterDot]", y, " + ", c, ")/(", a, "^2 + ", b, "^2)"];
                ...
                Module[{expr, denominator, numerator, simplifiedExpr},
                    ...
                    (* U\[ZHacek] nezjednodu\[SHacek]ujeme zlomky, aby sa zachoval presn\[YAcute] form\[AAcute]t *)
                    (* Je odstr\[AAcute]nen\[AAcute] logika na zjednodu\[SHacek]enie zlomkov, aby v\[YAcute]razy zostali v p\[OHat]vodnej forme *)
                    ;
                ...
                Print["y' = y - 2b(ax + by + c)/(a^2 + b^2)"];
                Print["V tejto rovnici dop\[IAcute]\[SHacek]eme na\[SHacek]e hodnoty:"];
                Print["y' = ", y, " - 2\[CenterDot]", b, "\[CenterDot](", a, "\[CenterDot]", x, " + ", b, "\[CenterDot]", y, " + ", c, ")/(", a, "^2 + ", b, "^2)"];
                ...
                Print[Style["\nOverenie:", Bold]];
                Print["Ak aplikujeme symetriu e\[SHacek]te raz na v\[YAcute]sledn\[YAcute] bod [", newX, ", ", newY, "], dostaneme p\[OHat]vodn\[YAcute] bod [", x, ", ", y, "]"];
            ],
            {i, 4}
        ];
        ...
        Print["Vrcholy \[SHacek]tvorca po symetrii:"];
        ...
        (* LEGENDA *)
        Print[Style["\nLEGENDA:", Bold, 14]];
        Print["\[Bullet] Modr\[EAcute] body a \[CHacek]iary: P\[OHat]vodn\[YAcute] \[SHacek]tvorec"];
        Print["\[Bullet] \[CapitalCHacek]erven\[EAcute] body a \[CHacek]iary: \[CapitalSHacek]tvorec po symetrii"];
        ...
        Table[
            Print[Row[{Style["Vrchol " <> FromCharacterCode[64 + i] <> "': ", 
                RGBColor[1, 0.1, 0.1]], outputVertices[[i]]}]], {i, 4}];
        ...
        (* \[CapitalSHacek]peci\[AAcute]lne vlastnosti symetri\[IAcute] \[SHacek]tvorca *)
        Print[Style["\n\[CapitalSHacek]PECI\[CapitalAAcute]LNE VLASTNOSTI SYMETRI\[CapitalIAcute] \[CapitalSHacek]TVORCA:", Bold, 14]];
        Print["\[Bullet] \[CapitalSHacek]tvorec m\[AAcute] \[SHacek]tyri osi symetrie: dve s\[UAcute] jeho uhloprie\[CHacek]ky a dve s\[UAcute] stredov\[EAcute] priamky rovnobe\[ZHacek]n\[EAcute] so stranami"];
        Print["\[Bullet] Ka\[ZHacek]d\[AAcute] os symetrie \[SHacek]tvorca prech\[AAcute]dza dvoma oproti\:013eehl\[YAcute]mi stranami alebo vrcholmi"];
        Print["\[Bullet] V\[SHacek]etky osi symetrie \[SHacek]tvorca sa pret\[IAcute]naj\[UAcute] v jeho strede (stred \[SHacek]tvorca je stred symetrie)"];
        Print["\[Bullet] Pri symetrii pod\:013ea uhloprie\[CHacek]ky sa vrcholy le\[ZHacek]iace na uhloprie\[CHacek]ke nemenia"];
        Print["\[Bullet] Pri symetrii pod\:013ea stredov\[YAcute]ch priamok sa vrcholy \[SHacek]tvorca zobrazia na oproti\:013eehl\[EAcute] vrcholy"];
        Print["\[Bullet] \[CapitalSHacek]tvorec m\[AAcute] 8 symetri\[IAcute] (4 osov\[EAcute] symetrie, rota\[CHacek]n\[UAcute] symetriu, identitu a stredov\[UAcute] symetriu)"];
        Print["\[Bullet] Symetrie \[SHacek]tvorca tvoria matematick\[UAcute] \[SHacek]trukt\[UAcute]ru zvan\[UAcute] diehedr\[AAcute]lna grupa D4"];
        
        (* V\[YAcute]znamn\[EAcute] typy os\[IAcute] symetrie *)
        Print[Style["\nV\[CapitalYAcute]ZNAMN\[CapitalEAcute] OSI SYMETRIE \[CapitalSHacek]TVORCA:", Bold, 14]];
        Print["\[Bullet] Os x (y = 0): bod [x, y] sa zobraz\[IAcute] na [x, -y]"];
        Print["\[Bullet] Os y (x = 0): bod [x, y] sa zobraz\[IAcute] na [-x, y]"];
        Print["\[Bullet] Priamka y = x: bod [x, y] sa zobraz\[IAcute] na [y, x] - prv\[AAcute] uhloprie\[CHacek]ka \[SHacek]tvorca"];
        Print["\[Bullet] Priamka y = -x: bod [x, y] sa zobraz\[IAcute] na [-y, -x] - druh\[AAcute] uhloprie\[CHacek]ka \[SHacek]tvorca"];
        Print["\[Bullet] Vodorovn\[AAcute] os symetrie \[SHacek]tvorca: bod [x, y] sa zobraz\[IAcute] na [x, 2k - y], kde k je y-ov\[AAcute] s\[UAcute]radnica stredu \[SHacek]tvorca"];
        Print["\[Bullet] Zvisl\[AAcute] os symetrie \[SHacek]tvorca: bod [x, y] sa zobraz\[IAcute] na [2h - x, y], kde h je x-ov\[AAcute] s\[UAcute]radnica stredu \[SHacek]tvorca"];
...
StvorecSymetria::invalidInput = "Neplatn\[YAcute] vstup. O\[CHacek]ak\[AAcute]va sa matica alebo zoznam s\[UAcute]radn\[IAcute]c \[SHacek]tvorca.";
StvorecSymetria::insufficientVertices = "Nedostato\[CHacek]n\[YAcute] po\[CHacek]et vrcholov. S\[UAcute] potrebn\[EAcute] aspo\[NHacek] 4 vrcholy.";
