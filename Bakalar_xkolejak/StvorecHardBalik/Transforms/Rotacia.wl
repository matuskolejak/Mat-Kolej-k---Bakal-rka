...
BeginPackage["StvorecHardBalik`Transforms`Rotacia`", {"StvorecHardBalik`"}];

(* Export public symbols *)
StvorecRotacia::usage = 
    "StvorecRotacia[vertices_] zobraz\[IAcute] interakt\[IAcute]vny tutori\[AAcute]l pre rot\[AAcute]ciu \[SHacek]tvorca okolo po\[CHacek]iatku a vr\[AAcute]ti nov\[EAcute] vrcholy.";
...
NormalizeVertices[vertices_] := 
    Module[{processedVertices},
                ...
                If[Length[DeleteDuplicates[Length /@ vertices]] > 1,
                    ...
                    Block[{cleanedVertices = Select[vertices, VectorQ]},
                        If[Length[cleanedVertices] >= 4,
                            cleanedVertices[[1;;4]],
                            Message[StvorecRotacia::invalidInput]; 
                            Abort[]
                        ]
                    ],
        ...
        If[Length[processedVertices] < 4,
            Message[StvorecRotacia::insufficientVertices];
            Abort[]
        ];
        
        (* Kontrola, \[CHacek]i vrcholy zostan\[UAcute] v bezpe\[CHacek]nej oblasti, ale zachov\[AAcute]vame presn\[EAcute] hodnoty *)
        If[Max[Abs[N[processedVertices[[All, 1]]]]] > 50 || Max[Abs[N[processedVertices[[All, 2]]]]] > 50,
            ...
            processedVertices = processedVertices / (Max[Abs[N[Flatten[processedVertices]]]] / 25);
        ];
        
        (* NEBUDEME ZAOKR\[CapitalUAcute]H\:013dOVA\[CapitalTHacek] - zachov\[AAcute]me presn\[EAcute] hodnoty *)
        
        (* N\[AAcute]vrat prv\[YAcute]ch \[SHacek]tyroch vrcholov *)
...
(* Funkcia na presn\[YAcute] v\[YAcute]po\[CHacek]et rot\[AAcute]cie bodu - OPRAVEN\[CapitalAAcute] VERZIA *)
ExactRotatePoint[point_, angle_] := 
    Module[{x, y, cosA, sinA, newX, newY},
        (* Extrahujeme x a y s\[UAcute]radnice vstupn\[EAcute]ho bodu *)
        x = point[[1]];
        y = point[[2]];
        
        (* Z\[IAcute]skame presn\[EAcute] hodnoty s\[IAcute]nus a kos\[IAcute]nus pre dan\[YAcute] uhol *)
        cosA = GetExactTrigValue[angle, Cos];
        sinA = GetExactTrigValue[angle, Sin];
        
        (* Aplikujeme \[SHacek]tandardn\[UAcute] rota\[CHacek]n\[UAcute] maticu:
           [cos(\[Alpha]), -sin(\[Alpha])]
           [sin(\[Alpha]),  cos(\[Alpha])]  *)
        newX = x*cosA - y*sinA;
        newY = x*sinA + y*cosA;
        
        (* Zachov\[AAcute]me presn\[EAcute] hodnoty bez zbyto\[CHacek]n\[EAcute]ho zjednodu\[SHacek]ovania,
           \[CHacek]o by mohlo vies\[THacek] k strate presnosti *)
        {newX, newY}
    ];
...
(* Jednoduch\[SHacek]ia a spo\:013eahlivej\[SHacek]ia funkcia na generovanie parametrov rot\[AAcute]cie *)
GetRotationParameters[vertices_] := 
    Module[{angle, simplifiedAngles, selectedAngle, newVertices, popis, popisUhla, kosinus, sinus},
        
        (* Preferujeme jednoduch\[EAcute] uhly, ktor\[EAcute] maj\[UAcute] exaktn\[EAcute] hodnoty s\[IAcute]nus a kos\[IAcute]nus *)
        simplifiedAngles = {30, 45, 60, 90, 120, 135, 150, 180};
        
        (* Vyberieme n\[AAcute]hodn\[YAcute] uhol *)
        selectedAngle = RandomChoice[simplifiedAngles];
        angle = selectedAngle;
        ...
        (* Vr\[AAcute]time v\[YAcute]sledok *)
...
CreateVisualization[originalVertices_, finalVertices_, angle_] := 
    Module[{allVertices, xMin, xMax, yMin, yMax, rangeBuffer, xRange, yRange, 
        ...
        Graphics[{
            ...
            (* Rotovan\[YAcute] \[SHacek]tvorec - \[CHacek]erven\[YAcute] *)
...
StvorecRotacia[inputVertices_] := 
    Module[{normalizedVertices, angle, popis, outputVertices, cosA, sinA, invAngle, cosInvA, sinInvA,
            popisUhla, expandedOutputVertices, rotationParams},
            
        (* Normalizujeme vstupn\[EAcute] vrcholy bez zmeny ich poz\[IAcute]cie *)
        normalizedVertices = NormalizeVertices[inputVertices];
        
        (* Z\[IAcute]skame parametre rot\[AAcute]cie - pomocou zjednodu\[SHacek]enej funkcie *)
        rotationParams = GetRotationParameters[normalizedVertices];
        angle = rotationParams[[1]];
        popis = rotationParams[[2]];
        outputVertices = rotationParams[[3]];
        cosA = rotationParams[[4]];
        sinA = rotationParams[[5]];
        
        (* Inverzn\[YAcute] uhol pre overenie spr\[AAcute]vnosti *)
        invAngle = -angle;
        ...
        (* Nadpis *)
        Print[Style[
            "GEOMETRICK\[CapitalEAcute] TRANSFORM\[CapitalAAcute]CIE \[CapitalSHacek]TVORCA - ROT\[CapitalAAcute]CIA", 
            Bold, 16]];
        Print[Style["========================================", Bold]];
        ...
        Print["\nVykonajte rot\[AAcute]ciu \[SHacek]tvorca v 2D priestore okolo po\[CHacek]iatku s\[UAcute]radnicovej s\[UAcute]stavy [0,0] o uhol \[Alpha] = ", 
        ...
        Print[Style["Transforma\[CHacek]n\[AAcute] matica rot\[AAcute]cie:", Bold]];
        Print[MatrixForm[{
            {Style["cos(\[Alpha])", Red], Style["-sin(\[Alpha])", Red]}, 
            {Style["sin(\[Alpha])", Red], Style["cos(\[Alpha])", Red]}
        }]];
        
        Print["\nNa\[SHacek]a hodnota \[Alpha] = ", angle, "\[Degree]:"];
        ...
        Table[
            Module[{x = normalizedVertices[[i, 1]], 
                ...
                (* Zjednodu\[SHacek]enie v\[YAcute]razov pre lep\[SHacek]iu \[CHacek]itate\:013enos\[THacek] *)
        ...
        Print["Vrcholy \[SHacek]tvorca po rot\[AAcute]cii:"];
        
        (* Plne rozvinut\[AAcute] matica v\[YAcute]sledku *)
        ...
        Print[MatrixForm[Map[Style[#, Red] &, expandedOutputVertices, {2}]]];
        ...
        Print["\[Bullet] \[CapitalCHacek]erven\[EAcute] body a \[CHacek]iary: Oto\[CHacek]en\[YAcute] \[SHacek]tvorec"];
        ...
        Table[
            Print[Row[{Style["Vrchol " <> FromCharacterCode[64 + i] <> "': ", 
                RGBColor[1, 0.1, 0.1]], expandedOutputVertices[[i]]}]], {i, 4}];
        ...
        Print["\[Bullet] Rot\[AAcute]cia zachov\[AAcute]va obsah a obvod \[SHacek]tvorca"];
        ...
        (* Vracame v\[YAcute]sledok ako rotovan\[EAcute] vrcholy *)
        expandedOutputVertices
    ];

(* Defin\[IAcute]cia chybov\[YAcute]ch spr\[AAcute]v *)
StvorecRotacia::invalidInput = "Neplatn\[YAcute] vstup. O\[CHacek]ak\[AAcute]va sa matica alebo zoznam s\[UAcute]radn\[IAcute]c \[SHacek]tvorca.";
StvorecRotacia::insufficientVertices = "Nedostato\[CHacek]n\[YAcute] po\[CHacek]et vrcholov. S\[UAcute] potrebn\[EAcute] aspo\[NHacek] 4 vrcholy.";
