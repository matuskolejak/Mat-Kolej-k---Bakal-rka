...
BeginPackage["StvorecEasyBalik`"];

(* Export verejn\[YAcute]ch symbolov *)
StvorecJednoduchaTransformacia::usage = 
    "StvorecJednoduchaTransformacia[] umo\[ZHacek]n\[IAcute] v\[YAcute]ber a aplik\[AAcute]ciu jednej transform\[AAcute]cie.";

StvorecJednoduchaTransformaciaSVysledkom::usage = 
    "StvorecJednoduchaTransformaciaSVysledkom[] umo\[ZHacek]n\[IAcute] v\[YAcute]ber a aplik\[AAcute]ciu jednej transform\[AAcute]cie a zobraz\[IAcute] s\[UAcute]hrnn\[YAcute] v\[YAcute]sledok.";
...
StvorecJednoduchaTransformacia::infinityerr = 
...
GenerateInitialSquare[] := Module[{
  ...
  result = Check[
    Block[{$MessagePreprint = (Message[StvorecJednoduchaTransformacia::infinityerr]; #)&},
      While[True,
...
DisplayTransformationSequence[pociatocny_, finalny_, transformacia_] := Module[{},
    ...
    DisplaySquareProperties[finalny, mildGreen];
];

(* Funkcia pre vizualiz\[AAcute]ciu jednej transform\[AAcute]cie *)
CreateSquareVisualization[pociatocny_, finalny_, transformacia_] := 
    Module[{
        ...
        labelPositions = Table[
            ...
            {i, 8}  (* 4 vertices per square, 2 squares *)
        ];
            ...
            Table[
                {
                    ...
                    brightGreen, Disk[finalny[[i]], 0.12]
                },
                {i, 4}
            ],
        ...
        ImageSize -> 650,
        PlotLabel -> Style["Aplik\[AAcute]cia transform\[AAcute]cie \[SHacek]tvorca", Bold, 16],
...
StvorecJednoduchaTransformacia[] := Module[{
    pociatocnyStvorec,
    transformacia,
    finalnyStvorec,
    transformaciaPopis
    ...
    Print[Style["JEDNODUCH\[CapitalAAcute] GEOMETRICK\[CapitalAAcute] TRANSFORM\[CapitalAAcute]CIA \[CapitalSHacek]TVORCA", Bold, 24]];
    Print[Style["==========================================", Bold]];
    ...
    finalnyStvorec = Switch[transformacia,
    ...
    Print[Style["\[Bullet] Tmavozelen\[EAcute]", RGBColor[0.2, 0.7, 0.3], Bold], " body a \[CHacek]iary: \[CapitalSHacek]tvorec A'B'C'D' po transform\[AAcute]cii (", 
    ...
    {pociatocnyStvorec, finalnyStvorec, transformacia}
...
StvorecJednoduchaTransformaciaSVysledkom[] := Module[{result},
   (* Pou\[ZHacek]itie Quiet na potla\[CHacek]enie chybov\[YAcute]ch spr\[AAcute]v *)
   result = Quiet[StvorecJednoduchaTransformacia[], {Power::infy, Infinity::indet}];
...
EndPackage[]; (* Uzavretie StvorecEasyBalik package *)
