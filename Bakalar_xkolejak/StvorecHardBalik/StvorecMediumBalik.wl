...
BeginPackage["StvorecMediumBalik`"];

(* Export verejn\[YAcute]ch symbolov *)
StvorecDvojitaTransformacia::usage = 
    "StvorecDvojitaTransformacia[] umo\[ZHacek]n\[IAcute] postupn\[YAcute] v\[YAcute]ber a aplik\[AAcute]ciu dvoch transform\[AAcute]ci\[IAcute].";

StvorecDvojitaTransformaciaSVysledkom::usage = 
    "StvorecDvojitaTransformaciaSVysledkom[] umo\[ZHacek]n\[IAcute] postupn\[YAcute] v\[YAcute]ber a aplik\[AAcute]ciu dvoch transform\[AAcute]ci\[IAcute] a zobraz\[IAcute] s\[UAcute]hrnn\[YAcute] v\[YAcute]sledok.";
...
StvorecDvojitaTransformacia::infinityerr = 
...
StvorecPosun = StvorecHardBalik`Transforms`Posun`StvorecPosun;
StvorecRotacia = StvorecHardBalik`Transforms`Rotacia`StvorecRotacia;
StvorecZvacsenieZmensenie = StvorecHardBalik`Transforms`ZvacsenieZmensenie`StvorecZvacsenieZmensenie;
StvorecSkosenie = StvorecHardBalik`Transforms`Skosenie`StvorecSkosenie;
StvorecSymetria = StvorecHardBalik`Transforms`Symetria`StvorecSymetria;
...
GenerateInitialSquare[] := Module[{
  ...
  result = Check[
    Block[{$MessagePreprint = (Message[StvorecDvojitaTransformacia::infinityerr]; #)&},
      While[True,
...
DisplayTransformationSequence[pociatocny_, druhy_, finalny_, prva_, druha_] := Module[{},
    ...
    DisplaySquareProperties[finalny, Orange];
];

(* Funkcia pre vizualiz\[AAcute]ciu dvoch transform\[AAcute]ci\[IAcute] *)
CreateSquareVisualization[pociatocny_, druhy_, finalny_, prva_, druha_] := 
    Module[{
        ...
        labelPositions = Table[
            ...
            {i, 12}  (* 4 vertices per square, 3 squares *)
        ];
        ...
        ImageSize -> 650,
        PlotLabel -> Style["Postupn\[AAcute] aplik\[AAcute]cia dvoch transform\[AAcute]ci\[IAcute] \[SHacek]tvorca", Bold, 16],
...
StvorecDvojitaTransformacia[] := Module[{
    pociatocnyStvorec,
    prvaTransformacia,
    druhyStvorec,
    druhaTransformacia,
    ...
    Print[Style["DVOJIT\[CapitalAAcute] GEOMETRICK\[CapitalAAcute] TRANSFORM\[CapitalAAcute]CIA \[CapitalSHacek]TVORCA", Bold, 24]];
    Print[Style["==========================================", Bold]];
    ...
    finalnyStvorec = Switch[druhaTransformacia,
    ...
    Print[Style["\[Bullet] Tmavozelen\[EAcute]", RGBColor[0.2, 0.7, 0.3], Bold], " body a \[CHacek]iary: \[CapitalSHacek]tvorec A'B'C'D' po prvej transform\[AAcute]cii (", 
    ...
    {pociatocnyStvorec, druhyStvorec, finalnyStvorec, 
...
StvorecDvojitaTransformaciaSVysledkom[] := Module[{result},
   (* Pou\[ZHacek]itie Quiet na potla\[CHacek]enie chybov\[YAcute]ch spr\[AAcute]v *)
   result = Quiet[StvorecDvojitaTransformacia[], {Power::infy, Infinity::indet}];
...
EndPackage[]; (* Uzavretie StvorecMediumBalik package *)
