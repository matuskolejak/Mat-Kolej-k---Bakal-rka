...
(* ::Package:: *)
(* In ZvacsenieZmensenie.wl - Make sure the global variable is correctly defined and used *)
...
ZvacsenieParams::usage = "ZvacsenieParams je globalny vektor {sx, sy} pre koeficienty zvacsenia/zmensenia, ktory sa pouziva v DomcekZvacsenieZmensenie funkcii.";
    ...
    Module[{sx, sy, foundScaling = False, detectedSx, detectedSy, i = 1, point, scaledPoint},
        While[i <= 5 && !foundScaling,
            ...
            If[point[[1]] != 0 && scaledPoint[[1]] != 0,
                ...
                detectedSx = EnsureExactRational[scaledPoint[[1]]/point[[1]]];
                foundScaling = True;
            ];
            
            If[point[[2]] != 0 && scaledPoint[[2]] != 0,
                
                detectedSy = EnsureExactRational[scaledPoint[[2]]/point[[2]]];
                foundScaling = True;
            ];
        ...
        If[foundScaling, 
            ...
            {EnsureExactRational[Simplify[detectedSx]], EnsureExactRational[Simplify[detectedSy]]}, 
            None
        ]
    ];
    ...
    Module[{sx, sy, citatel, menovatel, typTransformacie, popis, valid = False, narocnost},
        ...
        While[!valid,
            ...
            If[narocnost < 0.7,
                ...
                sx = EnsureExactRational[citatel/menovatel];
                
                citatel = RandomChoice[{1, 2, 3}];
                menovatel = RandomChoice[{1, 2, 3}];
                sy = EnsureExactRational[citatel/menovatel];
                
                
                If[IntegerQ[sx] && IntegerQ[sy],
                    If[RandomReal[] < 0.5,
                        sx = EnsureExactRational[1/sx],
                        sy = EnsureExactRational[1/sy]
                    ]
                ],
                ...
                citatel = RandomChoice[{1, 2, 3}];
                menovatel = RandomChoice[{1, 2, 3}];
                sx = Rationalize[citatel/menovatel, 0];
                
                citatel = RandomChoice[{1, 2, 3}];
                menovatel = RandomChoice[{1, 2, 3}];
                sy = Rationalize[citatel/menovatel, 0];
            ];
...
EnsureExactRational[value_] := If[NumericQ[value], Rationalize[value, 0], value];
...
DomcekZvacsenieZmensenieNoDisplayWithParams[inputVertices_] := 
    Module[{sx, sy, typTransformacie, popis, spolPodst, finalVertices, 
        ...
        If[detectedScaling =!= None,
            sx = Rationalize[detectedScaling[[1]], 0];  
            sy = Rationalize[detectedScaling[[2]], 0];  
            {typTransformacie, popis, spolPodst} = 
                DomcekHardBalik`Transforms`ZvacsenieZmensenie`IdentifyTransformationType[sx, sy];
            ...
            If[DomcekHardBalik`Transforms`ZvacsenieZmensenie`ZvacsenieParams =!= {0, 0},
                ...
                sx = EnsureExactRational[sx];
                sy = EnsureExactRational[sy];
                ...
                {sx, sy, typTransformacie, popis, spolPodst} = 
                    DomcekHardBalik`Transforms`ZvacsenieZmensenie`GenerateScalingParameters[]
            ];
            
            
            finalVertices = Map[
                Function[point,
                    {EnsureExactRational[Simplify[point[[1]]*sx]], 
                     EnsureExactRational[Simplify[point[[2]]*sy]]}
                ],
                normalizedVertices
            ]
        ];
        ...
        DomcekHardBalik`Transforms`ZvacsenieZmensenie`ZvacsenieParams = {Rationalize[sx, 0], Rationalize[sy, 0]};
...
DomcekZvacsenieZmensenie[inputVertices_] := 
    Module[{sx, sy, typTransformacie, popis, spolPodst, finalVertices, 
        ...
        If[MatrixQ[inputVertices, NumberQ],
            inputVertices = Map[EnsureExactRational, inputVertices, {2}]
        ];
        ...
        If[detectedScaling =!= None,
            ...
            sx = EnsureExactRational[detectedScaling[[1]]];
            sy = EnsureExactRational[detectedScaling[[2]]];
            {typTransformacie, popis, spolPodst} = IdentifyTransformationType[sx, sy];
            finalVertices = detectedFinalVertices,
        ...
        Print[Style["\nVykonajte " <> ToString[spolPodst] <> " dom\[CHacek]eka pomocou transforma\[CHacek]nej matice v 2D priestore:", Bold]];
        Print[MatrixForm[{
        ...
        Table[
            Module[{x = normalizedVertices[[i, 1]], 
                ...
                resultVector = {EnsureExactRational[Simplify[x*sx]], EnsureExactRational[Simplify[y*sy]]};
        ...
        Print[Style["Transformovan\[YAcute] dom\[CHacek]ek A'B'C'D'E':", Red, Bold]];
        ...
        Print[Style["\[Bullet] \[CapitalCHacek]erven\[EAcute]", Red], " body a \[CHacek]iary: Transformovan\[YAcute] dom\[CHacek]ek"];
        ...
        ZvacsenieParams = {Rationalize[sx, 0], Rationalize[sy, 0]};
...
DomcekZvacsenieZmensenie::invalidInput = "Neplatn\[YAcute] vstup. O\[CHacek]ak\[AAcute]va sa matica alebo zoznam s\[UAcute]radn\[IAcute]c dom\[CHacek]eka.";
DomcekZvacsenieZmensenie::insufficientVertices = "Nedostato\[CHacek]n\[YAcute] po\[CHacek]et vrcholov. S\[UAcute] potrebn\[EAcute] aspo\[NHacek] 5 vrcholov.";
...
NormalizeVertices[vertices_] := 
    Module[{processedVertices},
                ...
                If[Length[DeleteDuplicates[Length /@ vertices]] > 1,
                    ...
                    Block[{cleanedVertices = Select[vertices, VectorQ]},
                        If[Length[cleanedVertices] >= 5,
                            cleanedVertices[[1;;5]],
                            Message[DomcekZvacsenieZmensenie::invalidInput]; 
                            Abort[]
                        ]
                    ],
        ...
        If[Length[processedVertices] < 5,
            Message[DomcekZvacsenieZmensenie::insufficientVertices];
            Abort[]
        ];
