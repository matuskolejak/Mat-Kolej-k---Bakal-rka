...
BeginPackage["DomcekMediumBalik`"];

(* Export verejn\[YAcute]ch symbolov *)
DomcekDvojitaTransformacia::usage = 
    "DomcekDvojitaTransformacia[] umo\[ZHacek]n\[IAcute] postupn\[YAcute] v\[YAcute]ber a aplik\[AAcute]ciu dvoch transform\[AAcute]ci\[IAcute].";

DomcekDvojitaTransformaciaSVysledkom::usage = 
    "DomcekDvojitaTransformaciaSVysledkom[] umo\[ZHacek]n\[IAcute] postupn\[YAcute] v\[YAcute]ber a aplik\[AAcute]ciu dvoch transform\[AAcute]ci\[IAcute] a zobraz\[IAcute] s\[UAcute]hrnn\[YAcute] v\[YAcute]sledok.";
...
DomcekDvojitaTransformacia::infinityerr = 
...
(* Vylep\[SHacek]en\[AAcute] funkcia na zobrazenie postupn\[EAcute]ho v\[YAcute]po\[CHacek]tu transform\[AAcute]ci\[IAcute] - PRE DVE TRANSFORM\[CapitalAAcute]CIE *)
DisplayTransformationSequence[pociatocny_, druhy_, finalny_, prva_, druha_] := Module[{},
    ...
    Print["Vrcholy dom\[CHacek]eka po prvej transform\[AAcute]cii:"];
    ...
    Print["Vrcholy dom\[CHacek]eka po druhej transform\[AAcute]cii:"];
    ...
    DisplayDomcekProperties[finalny, Orange];
];

(* Funkcia pre vizualiz\[AAcute]ciu dvoch transform\[AAcute]ci\[IAcute] *)
CreateDomcekVisualization[pociatocny_, druhy_, finalny_, prva_, druha_] := 
    Module[{
        ...
        labels2 = {"A'", "B'", "C'", "D'", "E'"},
        labels3 = {"A''", "B''", "C''", "D''", "E''"},
        originalHouseLines, secondHouseLines, finalHouseLines
    },
        ...
        (* Define house lines for all three houses *)
        ...
        *)
        ...
        (* Second house (green) *)
        secondHouseLines = {
            {druhy[[1]], druhy[[2]]},  (* A'-B': bottom line *)
            {druhy[[2]], druhy[[3]]},  (* B'-C': right side *)
            {druhy[[3]], druhy[[4]]},  (* C'-D': top line *)
            {druhy[[4]], druhy[[1]]},  (* D'-A': left side *)
            {druhy[[3]], druhy[[5]]},  (* C'-E': right roof *)
            {druhy[[4]], druhy[[5]]}   (* D'-E': left roof *)
        };
        ...
        labelPositions = Table[
            ...
            {i, 15}  (* 5 vertices per domcek, 3 domceky *)
        ];
        ...
        Graphics[{
            ...
            (* Second domcek - green - proper house shape *)
            {brightGreen, Thickness[0.005], Opacity[0.9], 
                Line /@ secondHouseLines
            },
            
            (* Final domcek - orange - proper house shape *)
            ...
            (* Vertex points - second domcek *)
            Table[{
                White, Disk[druhy[[i]], 0.20],
                brightGreen, Disk[druhy[[i]], 0.15]
            }, {i, 5}],
            
            (* Vertex points - final domcek *)
            Table[{
                White, Disk[finalny[[i]], 0.20],
                brightOrange, Disk[finalny[[i]], 0.15]
            }, {i, 5}],
            ...
            Table[
                With[{
                    label = Which[
                        ...
                        i <= 10, labels2[[i - 5]],       (* Second domcek - A' to E' *)
                        True, labels3[[i - 10]]          (* Final domcek - A'' to E'' *)
                    ],
                    color = Which[
                        i <= 5, brightBlue,
                        i <= 10, brightGreen,
        ...
        ImageSize -> 650,
        PlotLabel -> Style["Postupn\[AAcute] aplik\[AAcute]cia dvoch transform\[AAcute]ci\[IAcute] dom\[CHacek]eka", Bold, 16],
...
DomcekDvojitaTransformacia[] := Module[{
    pociatocnyDomcek,
    prvaTransformacia,
    druhyDomcek,
    druhaTransformacia,
    ...
    Print[Style["DVOJIT\[CapitalAAcute] GEOMETRICK\[CapitalAAcute] TRANSFORM\[CapitalAAcute]CIA DOM\[CapitalCHacek]EKA", Bold, 24]];
    Print[Style["==========================================", Bold]];
    ...
    finalnyDomcek = Switch[druhaTransformacia,
        "Posun", DomcekPosun[druhyDomcek],
        "Rot\[AAcute]cia", DomcekRotacia[druhyDomcek],
        "Zv\[ADoubleDot]\[CHacek]\[SHacek]enie/Zmen\[SHacek]enie", DomcekZvacsenieZmensenie[druhyDomcek],
        "Skosenie", DomcekSkosenie[druhyDomcek],
        "Symetria", DomcekSymetria[druhyDomcek]
    ];
    ...
    Print[Style["\[Bullet] Tmavozelen\[EAcute]", RGBColor[0.2, 0.7, 0.3], Bold], " body a \[CHacek]iary: Dom\[CHacek]ek po prvej transform\[AAcute]cii (", 
    ...
    {pociatocnyDomcek, druhyDomcek, finalnyDomcek, 
...
DomcekDvojitaTransformaciaSVysledkom[] := Module[{result},
   (* Pou\[ZHacek]itie Quiet na potla\[CHacek]enie chybov\[YAcute]ch spr\[AAcute]v *)
   result = Quiet[DomcekDvojitaTransformacia[], {Power::infy, Infinity::indet}];
...
EndPackage[]; (* Uzavretie DomcekMediumBalik package *)
