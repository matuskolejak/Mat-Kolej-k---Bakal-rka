...
TrojuholnikSymetriaMatrix::usage = 
    "TrojuholnikSymetriaMatrix[a_, b_, c_] returns the transformation matrix for symmetry with respect to line ax + by + c = 0.";


TrojuholnikSymetriaMatrix[a_, b_, c_] := 
    CalculateSymmetryMatrix[a, b, c];
    

SymetriaOs::usage = "SymetriaOs je glob\[AAcute]lna premenn\[AAcute] pre os symetrie {a, b, c}, ktor\[AAcute] sa pou\[ZHacek]\[IAcute]va v TrojuholnikSymetria funkcii.";

TrojuholnikSymetria::usage = 
    "TrojuholnikSymetria[vertices_] zobraz\[IAcute] interakt\[IAcute]vny tutorial pre symetriu trojuholn\[IAcute]ka pod\:013ea priamky a vr\[AAcute]ti nov\[EAcute] vrcholy.";

TrojuholnikSymetriaNoDisplayWithParams::usage = 
    "TrojuholnikSymetriaNoDisplayWithParams[vertices_] z\[IAcute]ska parametre symetrie a aplikuje ich bez zobrazovania.";
...
TrojuholnikSymetriaNoDisplayWithParams[vertices_] := 
    Module[{axis, result, lineParams, a, b, c},
        ...
        If[TrojuholnikHardBalik`Transforms`Symetria`SymetriaOs =!= Null,
            axis = TrojuholnikHardBalik`Transforms`Symetria`SymetriaOs;
        ,
            ...
            _, 
                ...
                If[ListQ[axis] && Length[axis] == 3,
                    ...
                    Module[{reflectFunc},
                        ...
                        reflectFunc = If[
                            MemberQ[Names["TrojuholnikHardBalik`Transforms`Symetria`Private`*"], 
                                  "ReflectPointOverLine"],
                            TrojuholnikHardBalik`Transforms`Symetria`Private`ReflectPointOverLine,
                    ...
                    Module[{defaultAxis = {1, 1, 0}},  
                        ...
                        Map[TrojuholnikHardBalik`Transforms`Symetria`Private`ReflectPointOverLine[
...
GenerateSymmetryLineFromGlobal[vertices_] := 
    Module[{a, b, c, lineType, popis, targetVertices, xIntercept, yIntercept},
        ...
        If[TrojuholnikHardBalik`Transforms`Symetria`SymetriaOs =!= Null,
            ...
            Switch[TrojuholnikHardBalik`Transforms`Symetria`SymetriaOs,
                "os x", 
                    a = 0; b = 1; c = 0;
                    lineType = "os x";
                ...
                _,
                    ...
                    If[ListQ[TrojuholnikHardBalik`Transforms`Symetria`SymetriaOs] && 
                       Length[TrojuholnikHardBalik`Transforms`Symetria`SymetriaOs] == 3,
                        {a, b, c} = TrojuholnikHardBalik`Transforms`Symetria`SymetriaOs;
                        lineType = "v\[SHacek]eobecn\[AAcute] priamka";
...
TrojuholnikSymetria[inputVertices_] := 
    Module[{normalizedVertices, symmetryParams, outputVertices, 
        ...
        TrojuholnikHardBalik`Transforms`Symetria`SymetriaMatrix = transformMatrix;
        ...
        Print[Style["Trojuholn\[IAcute]k po symetrii A'B'C':", Red, Bold]];
        ...
        Print[Style["\[Bullet] \[CapitalCHacek]erven\[EAcute]", Red], " body a \[CHacek]iary: Trojuholn\[IAcute]k po symetrii"];
